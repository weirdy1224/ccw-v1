...
function MultiSelect({ options, selected, onChange, placeholder }) {
  ...
  const handleSelectAllFiltered = () => {
    const filteredIds = filtered.map(o => o.ccps_id);
  ...
  const isAllFilteredSelected = filtered.length > 0 && filtered.every(o => selected.includes(o.ccps_id));
      ...
      {open && (
        <div className="pa-multiselect-dropdown">
          ...
          <div className="pa-multiselect-options">
            ...
            {filtered.map(option => (
              <label
                className={`pa-multiselect-option${selected.includes(option.ccps_id) ? ' pa-multiselect-option--selected' : ''}`}
                key={option.ccps_id}
              >
                <input
                  type="checkbox"
                  checked={selected.includes(option.ccps_id)}
                  onChange={() => handleSelect(option.ccps_id)}
                  tabIndex={-1}
                />
                <span>
                  {option.username}
...
const CCPSAssignments = () => {
  ...
  const officerStats = useMemo(() =>
    CCPSUsers.map(user => {
      const assigned = requests.filter(r => r.assigned_to === user.ccps_id); // use ccps_id here
  ...
  const filteredOfficers = useMemo(
    () => (selectedOfficers.length
      ? officerStats.filter(u => selectedOfficers.includes(u.ccps_id))
      : officerStats),
      ...
      <div className="pa-table-scroll">
        <table className="pa-table">
          <thead>
            <tr>
              <th>CCPS User</th>
              <th onClick={() => setSort(prev => ({ by: "totalAssigned", asc: prev.by === "totalAssigned" ? !prev.asc : true }))} className="pa-sortable">
                Assigned {sort.by === "totalAssigned" ? (sort.asc ? "▲" : "▼") : ""}
              </th>
              <th onClick={() => setSort(prev => ({ by: "totalCompleted", asc: prev.by === "totalCompleted" ? !prev.asc : true }))} className="pa-sortable">
                Completed {sort.by === "totalCompleted" ? (sort.asc ? "▲" : "▼") : ""}
              </th>
              <th onClick={() => setSort(prev => ({ by: "totalPending", asc: prev.by === "totalPending" ? !prev.asc : true }))} className="pa-sortable">
                Pending {sort.by === "totalPending" ? (sort.asc ? "▲" : "▼") : ""}
              </th>
              <th onClick={() => setSort(prev => ({ by: "totalRejected", asc: prev.by === "totalRejected" ? !prev.asc : true }))} className="pa-sortable">
          ...
          <tbody>
            {sortedOfficers.length ? sortedOfficers.map(user => (
              <tr key={user.ccps_id} className="pa-table-row" onClick={() => openModal(user)}>
                <td>{user.username}</td>
      ...
      {modalUser && (
        <div className="pa-modal-overlay" onClick={closeModal}>
          <div className="pa-modal" onClick={e => e.stopPropagation()}>
            <h2>{modalUser.username} {modalUser.station_name ? <span className="pa-modal-station">({modalUser.station_name})</span> : ""}</h2>
