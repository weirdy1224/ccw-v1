...
export default function ContOverallReports() {
  ...
  useEffect(() => {
    axios
      .get("/api/controller/stations", { headers: { Authorization: `Bearer ${token}` } })
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("/api/controller/requests", { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
